...
local function RebindHutch(inst, hutch)
	hutch = hutch or TheSim:FindFirstEntityWithTag(inst.PlayerID .. "_hutch")
...
local function RespawnHutch(inst)
	StopRespawn(inst)
	RebindHutch(inst, TheSim:FindFirstEntityWithTag(inst.PlayerID .. "_hutch") or SpawnHutch(inst))
end
...
local function OnLoad(inst, data)
	...
	if data.PlayerID ~= nil then
		inst:AddTag(data.PlayerID .. "_fishbowl")
	...
	if data.PlayerName ~= nil then
		...
		inst.components.named:SetName(data.PlayerName .. "'s Star-Sky")
...
return Prefab("hfe_fishbowl", fn, assets)
