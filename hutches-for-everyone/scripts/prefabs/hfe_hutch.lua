...
local hutch_access = GetModConfigData("hutch_access", "hutches-for-everyone")
...
local LIGHT_FALLOFF = 0.4

--Puffy Hutch with lightbulb
local DIM_LIGHT_RADIUS = 2.1
local DIM_LIGHT_INTENSITY = 0.6
local DIM_LIGHT_FALLOFF = 0.55
...
local function OnReflectDamage(inst, data)
	...
	if spear ~= nil and spear.components.finiteuses ~= nil then
		spear.components.finiteuses:Use(spear.components.weapon ~= nil and spear.components.weapon.attackwear or 1)
	end
...
local function CreateGroundGlow(inst, rotrate)
	...
	local scale = (math.random() * 0.2 + 1.2) * (math.random() < 0.5 and 1 or -1)

	groundglow.Transform:SetScale(scale, scale, scale)
	groundglow.Follower:FollowSymbol(inst.GUID, "base_point", 0, 0, 0)

	groundglow:InitFX(inst, {
		rot = math.random(0, 359),
		rotrate = rotrate,
		alpha = math.random(),
		alphadir = math.random() < 0.5,
		alpharate = math.random() * 0.02 + 0.005,
	})
...
local function CheckBattery(inst)
	...
	if inst._musicbattery ~= musicbattery then
		if
			inst._musicbattery ~= nil
			and inst._musicbattery:IsValid()
			and inst._musicbattery.components.fueled ~= nil
		then
...
local forms = {
	CreateForm(
		"FUGU",
		{ "lightbattery", "pointy" },
		"hutch_pufferfish_build",
		"hutch_pufferfish.png",
		SetDimLight,
		nil
	),
...
local function SerializeFX(inst, data, islocal)
	...
	--[0.0, 1.0)
	local alpha = math.floor(data.alpha * 255 + 0.5)

	--[true, false]
	local alphadir = data.alphadir

	--[.005, .025)
	local alpharate = math.floor((data.alpharate - 0.005) / 0.02 * 63 + 0.5)
...
local function DeserializeFX(inst, data)
	...
	data.rotrate = (inst._rotrate:value() - 1) * 0.5
	data.alpha = inst._alpha:value() / 255
	data.alphadir = inst._alphadir:value()
	data.alpharate = inst._alpharate:value() / 63 * 0.02 + 0.005
end
...
return Prefab("hfe_hutch", create_hutch, assets, prefabs),
